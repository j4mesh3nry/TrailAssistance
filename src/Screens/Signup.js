...
const SignUp = () => {
  ...
  const { reloadData, signInAs } = usePortal();
  ...
  const submit = (e) => {
    ...
    reloadData();
    signInAs('student', { name: form.name, email: form.email });
    navigate('/dashboard');
  };
    ...
    <div className="t-auth">
      ...
      <aside className="t-auth-panel" aria-label="Campus">
        ...
        <div className="t-auth-panel-content">
          ...
          <p style={{ color: '#c6d3e8', marginTop: 10 }}>Create your student profile and land straight in your workspace — ready to file your first request.</p>
        </div>
      </aside>
      <main className="t-auth-form-wrap">
        <div className="t-card t-auth-card">
          ...
          <p style={{ fontSize: '0.84rem', color: 'var(--t-slate-500)', marginTop: 4 }}>Your profile is saved on this device.</p>
          <form onSubmit={submit} style={{ marginTop: 14 }}>
            <div className="t-field">
              <label className="t-label" htmlFor="su-name">Full name <span className="t-req">*</span></label>
              <div className="t-input-icon"><User size={16} aria-hidden="true" /><input id="su-name" className="t-input" required value={form.name} onChange={set('name')} placeholder="e.g. Juan Dela Cruz" /></div>
            </div>
            <div className="t-field">
            ...
            <div className="t-field">
              ...
              <div className="t-input-icon"><Lock size={16} aria-hidden="true" /><input id="su-pass" className="t-input" type="password" required value={form.password} onChange={set('password')} placeholder="Create a password" /></div>
            </div>
